...
from ulauncher.search.calc.CalcHistory import save_history_entry
...
class CalcResultItem(ResultItem):
    ...
    def on_enter(self, query):
        if self.result is not None:
            save_history_entry(query, self.result)
            return CopyToClipboardAction(str(self.result))

        return DoNothingAction()


class CalcHistoryItem(ResultItem):
    """Read-only entry shown when browsing calc history ("hist"/"history")."""

    # pylint: disable=super-init-not-called
    def __init__(self, expr: str, result: str):
        self.expr = expr
        self.result = result
    ...
    def get_description(self, query) -> str:
        return 'Enter to copy the result to the clipboard'
